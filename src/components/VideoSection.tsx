...
export default function VideoSection() {
  ...
  const [progress,  setProgress]  = useState(0);   // 0ÃÂ¢ÃÂÃÂ1
  const [muted,     setMuted]     = useState(true);
  const [playing,   setPlaying]   = useState(false);
  const [entered,   setEntered]   = useState(false);

  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ rAF scroll tracker ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  useEffect(() => {
    let raf = 0;
    const tick = () => {
      ...
      if (el) {
        ...
        // 0 when section top enters bottom of screen ÃÂ¢ÃÂÃÂ 1 when section bottom exits top
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ autoplay when in view ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  useEffect(() => {
    if (!entered) return;
  ...
  /* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Derived values from progress ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */
  // Clip: starts fully closed (inset 50% each side), opens to 0 by progress=0.5
  const clipProgress = Math.min(1, progress / 0.5);
  const insetH = (1 - clipProgress) * 5;   // horizontal inset % (0ÃÂ¢ÃÂÃÂ5%)
  const insetV = (1 - clipProgress) * 12;  // vertical inset %   (0ÃÂ¢ÃÂÃÂ12%)
  const clip = `inset(${insetV}% ${insetH}% ${insetV}% ${insetH}% round 3px)`;

  // Scale: 0.92 ÃÂ¢ÃÂÃÂ 1.0
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            Tour pelo ImÃÂÃÂ³vel
          </p>
        </div>

        {/* ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ Video container ÃÂ¢ÃÂÃÂÃÂ¢ÃÂÃÂ */}
        <div style={{
          position: 'relative', zIndex: 10,
          ...
          <div style={{
            ...
            <p style={{
              ...
              <span style={{ color: '#B8965A' }}>TOUR ÃÂÃÂ· 4K</span>
            </p>
          </div>

          {/* Arc progress ÃÂ¢ÃÂÃÂ top right */}
          <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <button onClick={togglePlay} style={{
                ...
                {playing ? 'ÃÂ¢ÃÂÃÂ¸' : 'ÃÂ¢ÃÂÃÂ¶'}
              </button>
              <button onClick={toggleMute} style={{
                ...
                {muted ? 'ÃÂ°ÃÂÃÂÃÂ' : 'ÃÂ°ÃÂÃÂÃÂ'}
              </button>
              <span style={{
                fontFamily: "'Inter', sans-serif",
